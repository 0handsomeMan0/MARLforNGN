from bi_lstm_rnn_sd_agent import RNN_SD_Agent
...
class BasicMAC:
    ...
    def init_hidden(self, batch_size):
        if self.args.agent not in ['updet', 'transformer']:
            ...
            self.hidden_states = (self.agent.init_hidden().unsqueeze(0).expand(batch_size*2, self.n_agents, -1),
                                   self.agent.init_hidden().unsqueeze(0).expand(batch_size*2, self.n_agents, -1))
    ...
    def _build_agents(self, input_shape):
        self.agent = RNN_SD_Agent(input_shape, self.args)
